...
class AdminCommands(commands.Cog):
    ...
    async def has_admin_permissions(self, interaction: Interaction):
    ...
    async def clear_roles(self, interaction: Interaction, role: discord.Role):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
    ...
    async def ban(self, interaction: Interaction, user: discord.User, reason: str = None):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
    ...
    async def timeout(self, interaction: Interaction, member: discord.Member, time: str):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
    ...
    async def clear_timeout(self, interaction: Interaction, member: discord.Member):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
    ...
    async def kick(self, interaction: Interaction, user: discord.User, reason: str = None):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
    ...
    async def warn(self, interaction: Interaction, user: discord.User):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
    ...
    async def warnings(self, interaction: Interaction, user: discord.User):
        with open("data/warnings.json", "r") as f:
            warnings = json.load(f)
        await interaction.response.send_message(f"⚠️ {user} has {warnings.get(str(user.id), 0)} warnings.", ephemeral=True)

    async def remove_warning(self, interaction: Interaction, user: discord.User, count: int):
        if not await self.has_admin_permissions(interaction):
            await interaction.response.send_message("❌ You don't have permission.", ephemeral=True)
            return
        with open("data/warnings.json", "r") as f:
    ...
    async def restore_role(self, interaction: Interaction, user: discord.User):
